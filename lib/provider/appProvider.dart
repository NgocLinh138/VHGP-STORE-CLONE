...
import 'package:store_app/models/responseModel.dart';
...
class AppProvider with ChangeNotifier {
  ...
  void changeCluster(dynamic value) {
    print(clusterOfBuildingController.text.isEmpty);
  ...
  Future<List<Area>> getAreaList() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (area.isNotEmpty) {
          await getAreaById(int.parse(area[0].id));
        }
      } else if (response.statusCode == 404) {
        area = [];
      }
  ...
  Future<void> getAddress() async {
    ...
    var response = await http.get(
      Uri.parse(
          '${ApiServices.baseURL}/${ApiServices.BUILDING}?pageIndex=${1}&pageSize=${100}&AccountId=id'),
    );
  ...
  Future<void> submitOrder() async {
    ...
    await ResponseUtil.postMapping(ApiServices.ORDER, cartToJson(cart))
        .then((value) {
      try {
